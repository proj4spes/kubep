...
module "ca" {
  ...
  ip_addresses_list = "${concat(aws_instance.master.*.public_ip, aws_instance.worker.*.public_ip)}"
  ssh_user          = "core"
...
module "kube_apiserver_certs" {
  ...
  ca_cert_pem           = "${module.ca.ca_cert_pem}"
  ca_private_key_pem    = "${module.ca.ca_private_key_pem}"
  ip_addresses          = "${compact(aws_instance.master.*.public_ip)}"
...
module "kube_worker_certs" {
  ...
  ca_cert_pem           = "${module.ca.ca_cert_pem}"
  ca_private_key_pem    = "${module.ca.ca_private_key_pem}"
  ip_addresses          = "${compact(aws_instance.worker.*.public_ip)}"
